<?php
	...
	if(isset($_SESSION['admin_login'])) {
			// Show ALL products with total quantity from all manufacturers
			// Only count manufacturers who have quantity > 0
			$query_selectProducts = "SELECT p.*, u.unit_name, 
									COALESCE(SUM(ms.quantity), 0) as total_quantity,
									COUNT(DISTINCT CASE WHEN ms.quantity > 0 THEN ms.manufacturer_id END) as manufacturer_count
									FROM products p 
...
<body>
	...
	<section>
		...
		<table class="table_displayData">
			...
			<td> <strong><?php echo $row_selectProducts['total_quantity']; ?></strong> </td>
			<td> <?php echo $row_selectProducts['manufacturer_count']; ?> manufacturer(s) </td>
