<?php
	...
	if(isset($_SESSION['admin_login'])) {
		if($_SESSION['admin_login'] == true) {
			...
			$name = $price = $unit = $rdbStock = $description = "";
			...
			if($name != null && $price != null && $unit != null && $rdbStock == 1) {
				$rdbStock = 0;
				$query_addProduct = "INSERT INTO products(pro_name,pro_desc,pro_price,unit_id) VALUES('$name','$description','$price','$unit')";
				...
				if(mysqli_query($con,$query_addProduct)) {
					...
					$requireErr = "* All Fields are Compulsory with valid values except Description and Stock";
				}
			}
		}
		else {
...
<body>
	...
	<section>
		...
		<li>
			...
			<div class="input-box"> <input type="text" id="product:name" name="txtProductName" placeholder="e.g., Basmati Rice, Bengal Gram" value="<?php echo $nameHolder; ?>" required /> </div> <span class="error_message"><?php echo $nameErr; ?></span>
		</li>
		<li>
			<div class="label-block"> <label for="product:price">Price (৳ Taka)</label> </div>
			<div class="input-box"> <input type="text" id="product:price" name="txtProductPrice" placeholder="e.g., 120.50 (in Taka)" value="<?php echo $priceHolder; ?>" required /> </div> <span class="error_message"><?php echo $priceErr; ?></span>
		</li>
		<li>
		...
		<select name="cmbProductUnit" id="product:unit">
			...
			<option value="<?php echo $row_selectUnit["unit_id"]; ?>"> <?php echo $row_selectUnit["unit_name"]; ?> </option>
			<?php } ?>
		</select>
		</div>
		</li>
